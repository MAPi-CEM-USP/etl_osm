...
def fetch_and_process_features(city_geom, key, tags):
    ...
    # 3. Máscaras para separar tipos de geometria
    ...
    # 4. Transforma Polígonos em Centroides
    if is_polygon.any():
        gdf.loc[is_polygon, 'geometry'] = gdf.loc[is_polygon, 'geometry'].centroid
    
    # 5. Para linhas, mantém as originais E adiciona um ponto no meio
    if is_line.any():
    ...
    # 6. Retorna para Lat/Lon padrão e filtra os que estão estritamente dentro da cidade
...
def create_map(features_points, city_geom, key, columns_to_show=None):
    ...
    # Prepara o colormap
    unique_tags = sorted(features_points[key].unique())
...
# ==========================================
def save_files(m, features_points, save_path, key):
    """Salva os resultados em HTML, Parquet e PMTiles."""
    os.makedirs(save_path, exist_ok=True)
    
    # HTML
    html_file = f"docs/mapas/features_map_{key}.html"
    os.makedirs(os.path.dirname(html_file), exist_ok=True)
    m.save(html_file)
    print(f"Map saved: {html_file}")
    
    # Parquet
    pq_file = os.path.join(save_path, f"features_{key}.parquet")
    features_points.to_parquet(pq_file, compression="snappy")
    print(f"Parquet saved: {pq_file}")
    
    # PMTiles
    pmt_file = os.path.join(save_path, f"features_{key}.pmtiles")
    if os.path.exists(pmt_file): os.remove(pmt_file)
    features_points.to_file(pmt_file, driver="PMTiles", engine="pyogrio", MINZOOM=0, MAXZOOM=14, NAME=f"layer_{key}")
...
def process_key(key, tags, city_geom, save_path="Dados/Saída/", columns_to_show=None):
    ...
    save_files(m, features_points, save_path, key)
